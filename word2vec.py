...
from numpy import exp, log, dot, zeros, add, subtract, outer, random, dtype, float32 as REAL,\
...
if True:
    ...
    def train_sentence_sg(model, sentence, alpha, work=None):
        ...
        This is the non-optimized6, Python version. If you have cython installed, gensim
        ...
        for pos, word in enumerate(word_vocabs):
            ...
            for pos2, word2 in enumerate(word_vocabs[start:(pos + model.window + 1 - reduced_window)], start):
                # print "word "+str(model.index2word[word.index])
                # print "index "+str(word2.index)
    ...
    def train_sentence_cbow(model, sentence, alpha, work=None, neu1=None):
        ...
        word_vocabs = [model.vocab[w] for w in sentence if w in model.vocab and
                       model.vocab[w].sample_int > model.random.rand() * 2**32]
        for pos, word in enumerate(word_vocabs):
            logger.info("BEWARE YOU ARE TRAINING WITH CBOW WHICH IS NOT SUPPORTED IN SUBGRAM")
            reduced_window = model.random.randint(model.window)  # `b` in the original word2vec code
            start = max(0, pos - model.window + reduced_window)
...
def train_sg_pair(model, word, context_index, alpha, learn_vectors=True, learn_hidden=True,
    ...
    l1 = np_sum(model.syn0[model.vocab[word].substrings], axis=0)  # 1 x vector_size
    # the substrings contain also id of the word itself
    l1 /= len(model.vocab[word].substrings)
    ...
    if model.hs:
        ...
        # print str(len(l2a))+" "+str(len(l2a[0]))
        neu1e += dot(ga, l2a)  # save error

    if model.negative:
        logger.info("THIS PART OF CODE IS NOT SUPORTED BY SUBGRAM")
    ...
    if learn_vectors:
        # learn input -> hidden, here for all subgrams in the window separately
        #if not model.cbow_mean and input_word_indices:
        neu1e /= len(model.vocab[word].substrings)
        for i in model.vocab[word].substrings:
            #TODO i don't know what the lock is for
...
class Word2Vec(utils.SaveLoad):
    ...
    def __init__(
            ...
            trim_rule=None, sorted_vocab=1, min_count_sub=500):
        """
        ...
        self.min_count_sub = min_count_sub
        ...
        if heap:
            ...
            while stack:
                ...
                if node.index < len(self.vocab):
                    ...
                    #print str(node.index)+" "+ str(len(self.vocab)) +" - "+str(node.index - len(self.vocab))
    ...
    def scan_vocab(self, sentences, progress_per=10000, trim_rule=None):
        ...
        for word in vocab:
            self.word2substrings(word, substrings, 2, vocab[word], True)

        total_words += sum(itervalues(vocab))
        logger.info("collected %i word types from a corpus of %i raw words and %i sentences",
                    len(vocab), total_words, sentence_no + 1)
        logger.info("created %i substrings from %i words",
                    len(substrings),len(vocab))
        self.corpus_count = sentence_no + 1
        self.raw_vocab = vocab
        self.raw_substrings = substrings

    def word2substrings(self, word, substrings, length, occurence, abbrevite):
        if(abbrevite):
            word = "$"+word+"#";
        if length >= len(word):
            return
        ...
        for i in range(0, len(word)-length+1):
            s = word[i:i+length]
            substrings[s]+=occurence
        self.word2substrings(word, substrings, length+1, occurence, False)
    ...
    def scale_vocab(self, min_count=None, sample=None, dry_run=False, keep_raw_vocab=False, trim_rule=None):
        ...
        # Discard substring less-frequent than min_count_subs
        drop_unique_sub = 0
        min_count_sub = self.min_count_sub
        retain_subs = []
        for sub, v in iteritems(self.raw_substrings):
            if v >= min_count_sub:
                retain_subs.append(sub)
                #self.vocab[word] = Vocab(count=v, index=len(self.index2word))
                #self.index2word.append(word)
            else:
                drop_unique_sub += 1
        logger.info("min_count_sub=%d retains %i unique substrings (drops %i)",
                    min_count_sub, len(retain_subs), drop_unique_sub)
        self.raw_substrings=retain_subs

        #create substring2index
        sub2index = {}
        i=0
        for sub in retain_subs:
            sub2index[sub] = i;
            i+=1

        #insert indices of subgrams into vocab ... add size of vocabulary since all subgrams weights are after words
        for word in self.vocab:
            indices = []
            substrings = defaultdict(int)
            self.word2substrings(word, substrings, 2, 0, True)
            for sub in substrings:
                if sub in sub2index:
                    indices.append(sub2index[sub]+len(self.vocab))
            self.vocab[word].substrings = indices;
    ...
    def sort_vocab(self):
        ...
        for i, word in enumerate(self.index2word):
            ...
            # this is a hack because substrings contain even the index of a word
            self.vocab[word].substrings.append(i)
    ...
    def reset_weights(self):
        ...
        syn0size = len(self.vocab)
        syn0size+=len(self.raw_substrings) # the size must account subgrams
        self.syn0 = empty((syn0size, self.vector_size), dtype=REAL)
        ...
        self.syn0_lockf = ones(syn0size, dtype=REAL)  # zeros suppress learning
    ...
    def get_vector_representation(self, word):
        if word in self.vocab:
            if(self.vocab[word].substrings==1):
                l1 = self.syn0[self.vocab[word].index]
            else:


                l1 = np_sum(self.syn0[self.vocab[word].substrings], axis=0)  # 1 x vector_size
                l1 /= len(self.vocab[word].substrings) # the substrings contain also id of the word itself

        else:
            try:
                if(word in self.lookup_substings):
                    return self.lookup_substings[word]
            except AttributeError:
                self.lookup_substings = {}

            sub = defaultdict(int)
            self.word2substrings(word, sub, 2, 1, True)

            indices = []
            for sub, v in iteritems(sub):
                try:
                    indices.append(self.raw_substrings.index(sub))
                except ValueError:
                    pass


            l1 = np_sum(self.syn0[indices], axis=0)
            l1 /= len(indices)
            self.lookup_substings[word] = l1
        return l1
    ...
    def most_similar(self, positive=[], negative=[], topn=10, restrict_vocab=None):
        ...
        for word, weight in positive + negative:
            if isinstance(word, ndarray):
                ...
                #TODO this is only for SubGram
                mean.append(weight * self.get_vector_representation(word))
    ...
    def init_sims(self, replace=False):
        ...
        if getattr(self, 'syn0norm', None) is None or replace:
            ...
            if replace:
                ...
                syn0 = empty((len(self.vocab), self.vector_size), dtype=REAL)
                for word in self.vocab:
                    syn0[self.vocab[word].index] = self.get_vector_representation(word)
                    #syn0[self.vocab[word].index] = self.syn0[self.vocab[word].index]

                    ##vector = self.get_vector_representation(word)
                    ##self.syn0norm[self.vocab[word].index] = (vector/sqrt(sum(vector ** 2))).astype(REAL)
                self.syn0norm = (syn0 / sqrt((syn0 ** 2).sum(-1))[..., newaxis]).astype(REAL)

                # s = 0
                # for i in range(0,180239):
                #     for j in range(0,100):
                #         s = s + abs(a[i][j]-self.syn0norm[i][j])
                # print s
    ...
    def accuracy(self, questions, restrict_vocab=30000, most_similar=most_similar):
        ...
        for line_no, line in enumerate(utils.smart_open(questions)):
            ...
            if line.startswith(': '):
                ...
                ignore =[]
                if a in ok_vocab:
                    ignore.append(self.vocab[a].index)
                if b in ok_vocab:
                    ignore.append(self.vocab[b].index)
                if c in ok_vocab:
                    ignore.append(self.vocab[c].index)
